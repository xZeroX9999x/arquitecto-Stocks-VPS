...
def create_app() -> FastAPI:
    ...
    @app.get("/api/descubrimientos")
    async def obtener_descubrimientos():
        """Devuelve las joyas encontradas por el Minero."""
        try:
            conn = sqlite3.connect("data/arquitecto.db")
            cursor = conn.cursor()
            cursor.execute("SELECT ticker, fecha FROM descubrimientos ORDER BY fecha DESC")
            resultados = cursor.fetchall()
            conn.close()
            
            return {"status": "ok", "data": [{"ticker": fila[0], "fecha": fila[1]} for fila in resultados]}
